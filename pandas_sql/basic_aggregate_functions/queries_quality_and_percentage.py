...
def trans(d):
    return pd.Series(
        {'quality': round((d['rating'] / d['position']).mean(), 2),
         'poor_query_percentage': (len(d['query_name'].drop_duplicates()) / len(d)) * 100}
    )


df = queries_df.groupby('query_name').apply(trans).reset_index()
# Display the result
print(df)
